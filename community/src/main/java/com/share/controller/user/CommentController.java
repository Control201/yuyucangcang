...
public class CommentController {
    ...
    @PostMapping("/review")
    @ResponseBody
    public ReslutTypeDto review(@RequestBody ReplyDto replyDto, HttpServletRequest request) {
        ...
        //写入数据库成功，然后发送消息提醒，增加经验
        if (integer == 1) {
            // 增加经验 并且同步
            Integer exp = expUtils.addExp(uid, Exp.COMMENT_EXP);
